...
module.exports = {
  ...
  execute: async function(message, argsStr, embed) {
    ...
    const sql = 'SELECT * FROM test_set WHERE completed = false AND season = $1 ORDER BY id'
    const values = [season]
    const resSets = await db.query(sql, values)
    let sets = resSets.rows

    const sqlusers = 'SELECT * FROM test_points LEFT JOIN test_set ON set_id = id WHERE completed = false AND season = $1 ORDER BY set_id'
    ...
    if(argsStr.includes('all')) {
      ...
      if(sets.length === 0)
        return embed.setTitle((argsStr) ? `There are no incomplete games for ${user} during season ${season}` : `You have no incomplete games for season ${season}`)
