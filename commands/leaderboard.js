...
module.exports = {
  ...
  execute: async function(message, argsStr, embed) {
    ...
    const sqlAgg = 'SELECT COUNT(id), SUM(points), SUM(bonus) AS bonus, SUM(malus) AS malus, player_id FROM test_set INNER JOIN test_points ON set_id = id WHERE season = $1 AND completed = true GROUP BY player_id HAVING COUNT(id) >= 2'
